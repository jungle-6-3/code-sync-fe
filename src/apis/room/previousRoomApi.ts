import {
  GetPreviousRoomResponseDto,
  GetPreviousRoomsResponseDto,
  getPreviousShareRoomResponseDto,
} from "@/apis/conversation/dtos";
...
export const getPreviousRoom = async (conversationId: string) => {
  const { data } = await https.get<GetPreviousRoomResponseDto>(
...
export const patchPreviousRoom = async (
  dataPk: string,
  body: PatchConversationDatasRequest,
) => {
...
export const getPreviousShareRoom = async (shareId: string) => {
  const { data } = await https.get<getPreviousShareRoomResponseDto>(
    `/conversations/share/${shareId}`,
  );
  return data;
};
